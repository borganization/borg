...
#[allow(dead_code)] // scaffolding for the streaming markdown collector (#1)
...
/// Used by tool-result and exec cells to give the first row a tree glyph and
...
mod tests {
    ...
    #[test]
    fn truncate_line_preserves_style_on_ellipsis() {
        let red = Style::default().fg(Color::Red);
        let l = Line::from(vec![Span::raw("ok "), Span::styled("ERROR MESSAGE", red)]);
        let out = truncate_line_with_ellipsis(l, 6);
        // Last span should be the ellipsis, inherit red style from preceding span.
    ...
    #[test]
    fn truncate_line_fits_returns_unchanged() {
        let l = Line::from(vec![Span::raw("short")]);
        let out = truncate_line_with_ellipsis(l, 20);
        assert_eq!(line_width(&out), 5);
        assert!(!out.spans.iter().any(|s| s.content.contains('…')));
    }
