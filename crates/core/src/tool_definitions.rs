...
pub fn core_tool_definitions(config: &Config) -> Vec<ToolDefinition> {
    let mut defs = vec![
        ...
        ToolDefinition::new(tool_names::MEMORY_SEARCH, "Search memory files semantically, or past session transcripts with sources=[\"messages\"]. Use before answering questions about prior work, decisions, preferences, or anything previously discussed.", serde_json::json!({"type":"object","properties":{"query":{"type":"string","description":"Search query"},"max_results":{"type":"integer","description":"Maximum results to return (default: 5)","default":5},"min_score":{"type":"number","description":"Minimum relevance score 0-1 (default: 0.2)","default":0.2},"sources":{"type":"array","items":{"type":"string","enum":["memory","messages"]},"description":"Where to search. \"memory\" (default) searches curated long-term memory entries. \"messages\" searches raw past conversation transcripts via SQLite FTS5. Combine to search both."}},"required":["query"]})),
        ...
        ToolDefinition::new(tool_names::RUN_SHELL, "Execute a shell command. Do not use `sleep`, `at`, or background loops to emulate reminders or delayed tasks — use the `schedule` tool instead.", serde_json::json!({"type":"object","properties":{"command":{"type":"string","description":"Shell command to execute"}},"required":["command"]})),
...
fn build_schedule_tool_def(config: &Config) -> ToolDefinition {
    ...
    let description = if workflows_on {
        "Schedule reminders, one-shot tasks, recurring AI prompts, cron commands, and workflows. USE THIS whenever the user asks to be reminded, wants something to run later, or sets up any recurring automation (\"remind me tomorrow at 9am\", \"every weekday at 8\", \"in 30 minutes\", \"ping me when…\"). Do NOT emulate with run_shell sleep loops. type='prompt' = AI task, type='command' = shell cron, type='workflow' = multi-step orchestration. Actions: create, list, get, update, pause, resume, cancel, delete, runs, run_now."
    } else {
        "Schedule reminders, one-shot tasks, recurring AI prompts, and cron commands. USE THIS whenever the user asks to be reminded, wants something to run later, or sets up any recurring automation (\"remind me tomorrow at 9am\", \"every weekday at 8\", \"in 30 minutes\", \"ping me when…\"). Do NOT emulate with run_shell sleep loops. type='prompt' = AI task, type='command' = shell cron. Actions: create, list, get, update, pause, resume, cancel, delete, runs, run_now."
    };
    ...
    let mut properties = serde_json::json!({
        ...
        "schedule_type": {"type":"string","enum":["cron","interval","once"],"description":"For type=prompt create: 'once' = fire one time at a specific future datetime (use for reminders). 'interval' = repeat every fixed duration. 'cron' = 5-field cron expression."},
        "schedule_expr": {"type":"string","description":"For type=prompt create. once: ISO-8601 timestamp like '2026-04-22T09:00:00' (local tz if no offset) or '2026-04-22T09:00:00Z'. interval: '30m', '2h', '1d'. cron: '0 9 * * 1-5' (weekdays 9am)."},
