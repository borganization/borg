...
pub use crate::config::settings_table::{
    SETTING_REGISTRY, TUI_FLOAT_RANGES, TUI_SELECT_CHOICES, TUI_SETTINGS,
};

/// Return the static choice list for a `Select`-kind TUI setting, if one is
/// declared in `TUI_SELECT_CHOICES`. `provider` and `model` are dynamic and
/// return `None`.
pub fn tui_select_choices(key: &str) -> Option<&'static [&'static str]> {
    TUI_SELECT_CHOICES
        .iter()
        .find(|(k, _)| *k == key)
        .map(|(_, v)| *v)
...
/// Return the `(min, max, step)` tuple for a `Float`-kind TUI setting, if one
/// is declared in `TUI_FLOAT_RANGES`.
pub fn tui_float_range(key: &str) -> Option<(f64, f64, f64)> {
    TUI_FLOAT_RANGES
        .iter()
        .find(|(k, _, _, _)| *k == key)
        .map(|(_, min, max, step)| (*min, *max, *step))
}
...
mod tests {
    ...
    #[test]
    fn every_select_tui_setting_has_choices_or_is_dynamic() {
        // `provider` and `model` are dynamic (derived from the onboarding
        // provider/model catalog); everything else must declare a static
        // choice list so `cycle_select` can drive it without per-key code.
        let dynamic: std::collections::HashSet<&str> = ["provider", "model"].into_iter().collect();
        for decl in TUI_SETTINGS {
            if decl.kind != TuiSettingKind::Select {
                continue;
            }
            if dynamic.contains(decl.key) {
                continue;
            }
            assert!(
                tui_select_choices(decl.key).is_some(),
                "Select-kind setting '{}' has no entry in TUI_SELECT_CHOICES",
                decl.key
            );
        }
    }
    ...
    #[test]
    fn every_float_tui_setting_has_range() {
        for decl in TUI_SETTINGS {
            if decl.kind != TuiSettingKind::Float {
                continue;
            }
            assert!(
                tui_float_range(decl.key).is_some(),
                "Float-kind setting '{}' has no entry in TUI_FLOAT_RANGES",
                decl.key
            );
        }
    }
