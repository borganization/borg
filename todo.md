...
- [x] **Path traversal prevention in memory read/write** — `write_memory` and `read_memory` accept arbitrary filenames without sanitization, allowing writes/reads outside `~/.borg/` (e.g., `../../.bashrc`)
...
- [ ] **Deduplicate ToolDefinition types** — `borg_core::types::ToolDefinition` and `borg_tools::registry::borg_core_types::ToolDefinition` are identical but duplicated
...
- [ ] **Structured file logging** — log to `~/.borg/logs/` in addition to stderr
